...
public class RetroLookPro : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture src, RenderTexture dest)
    {
        ...
        if (tempPreset.enableColorPalette)
        {
            ...
            {
                Graphics.Blit(src, scaled, colorMat);
            }

            Graphics.Blit(scaled, src);
        ...
        if (tempPreset._enableTVmode)
            Graphics.Blit(src, zeroPass, tempPreset._bottomNoiseMat);
        else
        {
            Graphics.Blit(src, zeroPass, tempPreset._bottomNoiseMat);
            Graphics.Blit(zeroPass, src, tempPreset._bottomNoiseMat);
        }
        ...
        if (tempPreset._enableTVmode)
        {
            ...
            Graphics.Blit(tempPass, src, tempPreset._TV_Material);
        }
        //
        ...
        Graphics.Blit(src, texPass12, tempPreset.m_1);

        if (!tempPreset.a_Artefacts)
        {
            Graphics.Blit(texPass12, dest, tempPreset.m_2);
        }
        else
        {
